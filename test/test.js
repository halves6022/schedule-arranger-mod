...
describe('/schedules', () => {
  ...
  it('予定が作成でき、表示される', (done) => {
    User.upsert({ userId: 0, username: 'testuser', authProvider: 'testprovider' }).then(() => {
      request(app)
        .post('/schedules')
        .send({ scheduleName: 'テスト予定1', memo: 'テストメモ1\r\nテストメモ2', candidates: 'テスト候補1\r\nテスト候補2\r\nテスト候補3', openFlag: 'true' })
...
describe('/schedules/:scheduleId/users/:userId/candidates/:candidateId', () => {
  ...
  it('出欠が更新できる', (done) => {
    User.upsert({ userId: 0, username: 'testuser', authProvider: 'testprovider' }).then(() => {
      request(app)
        .post('/schedules')
        .send({ scheduleName: 'テスト出欠更新予定1', memo: 'テスト出欠更新メモ1', candidates: 'テスト出欠更新候補1', openFlag:'true' })
        .end((err, res) => {
...
// 更新データのupdateができない模様.そこでテストが中断する。 >>  && isMine(req, schedule)でフリーズする様子
describe('/schedules/:scheduleId?edit=1', () => {
  before(() => {
    passportStub.install(app);
    passportStub.login({ id: 0, username: 'testuser', authProvider: 'testprovider' });
  });
  ...
  it('予定が更新でき､候補が追加できる', (done) => {
    User.upsert({ userId: 0, username: 'testuser', authProvider: 'testprovider' }).then(() => {
      request(app)
        .post('/schedules')
        .send({ scheduleName: 'テスト更新予定1', memo: 'テスト更新メモ1', candidates: 'テスト更新候補1', openFlag: 'true' })
        .end((err, res) => {
          const createdSchedulePath = res.headers.location;
          const scheduleId = createdSchedulePath.split('/schedules/')[1];
          // 更新がされることをテスト
          request(app)
            ...
            .send({ scheduleName: 'テスト更新予定2', memo: 'テスト更新メモ2', candidates: 'テスト更新候補2', openFlag: 'false' })
            .end(() => {
              Schedule.findById(scheduleId).then((s) => {
                assert.equal(s.scheduleName, 'テスト更新予定2');
                assert.equal(s.memo, 'テスト更新メモ2');
                assert.equal(s.openFlag, false);
              });
              Candidate.findAll({
                where: { scheduleId: scheduleId },
...
// 終了処理に問題あり。テスト全て終わったあとにログインデータなどが消えずフリーズしたようになる
